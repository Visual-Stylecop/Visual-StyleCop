...
//
PWSTR   WINAPI  ToUpperCase    (_In_opt_z_count_(cch) PCWSTR pSrc, _Out_z_cap_(cch) PWSTR pDst, size_t cch);
PWSTR   WINAPI  ToLowerCase    (_In_opt_z_count_(cch) PCWSTR pSrc, _Out_z_cap_(cch) PWSTR pDst, size_t cch);
...
int     WINAPI  CompareNoCaseNPrepared (_In_opt_z_count_(cch) PCWSTR szA, _In_opt_z_ PCWSTR szB, size_t cch);
...
PCWSTR  WINAPI  StrStrNoCaseNPrepared  (_In_opt_count_(cch) PCWSTR text, size_t cch, _In_opt_z_ PCWSTR pattern);
...
inline WCHAR WINAPI LowerCase(WCHAR ch)
{
    ...
    // We can check for chars in the range 'A'-'Z' with a single unsigned comparison
    if (static_cast<unsigned short>(ch - L'A') <= static_cast<unsigned short>(L'Z' - L'A'))
    {
        return ch + (L'a' - L'A');
    }
